...
*Primera.R*. Si no sabe como hacerlo revise la ayuda de la función *save*, *history.save* 
...
```{r asignacion3, eval=FALSE, echo=TRUE}
sqrt()
```
...
*(No subestime esta funcion, es muy útil por ejemplo para generar intervalos para histogramas y etiquetas para plots)*
...
*¿Qué pasa con **z** que tiene diferente longitud?*
...
*¿Qué se obtiene si intento extraer la 4ta posición de esta variable (vector)? Como determino si una variable tiene valores ausentes?*


* Crear un vector numérico y asignarlo a **x**. Luego ordenar de manera creciente y  decreciente  y guardarlos en **cx**, **dx**. Usar *sort(), order()*
...
* Crear un vector lógico **L** y coercionarlo a valores numéricos usando *as.numeric()* y *as.double()*
...
Listar todos los objetos y variables que estan cargados en su sesión actual. 
...
Chequear si están instalados los paquetes: *knitr* y *markdown*. Si es asi, cargarlos. Si no, instalarlos.

Quitar los paquetes del entorno 

Cargar un paquete de su preferencia y chequear una función que este posea antes y 
despues de cargarlo. (ejemplo: paquete *limma*, función *VennDiagram*)

Instalar el paquete **xpose4** usando el comando *install.packages()*

Datos de paquetes instalados: R contiene muchos conjuntos de datos incorporados al software. Estos datos están guardados como *dataframes*. Para ver la lista de conjuntos de datos tipee *data()*. Se abrirá una ventana en la que se listan los conjuntos de datos disponibles (hay muchos otros en los paquetes escritos por usuarios).
...
Para acceder a las variables individuales en un *dataframe*, se usa un **$** entre el nombre del *dataframe* y el nombre de la columna:
...
**f3**: Crear una función que acepte un vector y una variable y devuelva el valor de  vector^variable. Si no se provee la variable, se devuelve el valor del vector^2
